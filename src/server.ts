...
- **\`happyhorse-1.1\`** — cost-effective alternative for both t2v and i2v, with auto-generated audio. Duration ~3–15s. Resolutions 720p / 1080p.
...
- **\`grok-video\`** — xAI Grok Imagine 1.5, currently the strongest image-to-video quality. **IMAGE-TO-VIDEO ONLY**: \`firstFrame\` is REQUIRED — a call without it (pure text-to-video) is rejected by the backend. Native audio included (always on, no param). Duration 4–15s (integer, default 4). Resolutions \`480p\` / \`720p\`. Output aspect ratio follows the first frame (no ratio param needed). Does NOT support text-to-video, last frame, or reference-video continuation — for those use seedance-2-0 / happyhorse-1.1 / veo-3.1.

Key rules:
- The \`model\` parameter is REQUIRED for \`generate_video\` (no platform default for video).
- \`grok-video\` is IMAGE-TO-VIDEO ONLY — \`firstFrame\` is mandatory; without it the backend rejects the request. If the user wants pure text-to-video, pick seedance-2-0 / happyhorse-1.1 / veo-3.1 instead.
